...
class GroundingEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        for input, output in zip(inputs, outputs):
            ...
            gt_display = input['groundings']['masks'].bool().int()[0]
            rand = random.randint(0,100)
            # print(pred.int())
            for mask in range(pred.shape[0]):

                cv2.imwrite(f'{mask}_output_{rand}.png', pred[mask].int().cpu().numpy()*255)
                cv2.imwrite(f'{mask}_gt_{rand}.png', gt_display.cpu().numpy() * 255)
            # c+=1
            # exit()
            # print(pred)
            # exit()
